...
async def _ensure_imap_started_for_user(uid: int, chat_id: int):
    """
    Старт IMAP‑воркеров для всех аккаунтов пользователя через новый imap_runtime.
    Используется для /read, imap:start_all и т.п.
    """
    accounts = await _get_user_accounts(uid)
    active_accounts = accounts  # без фильтра по несуществующему .active

    if not active_accounts:
        log_send_event(f"IMAP_RUNTIME: no accounts for uid={uid}, nothing to start")
        return
    ...
    for acc in active_accounts:
        ...
        try:
            ...
            log_send_event(
                f"IMAP_RUNTIME: get_account_proxy_async failed uid={uid} acc_id={acc.id} email={email}: {e}"
            )
            proxy = None
        ...
        try:
            ...
            log_send_event(
                f"IMAP_RUNTIME: failed to start worker uid={uid} acc_id={acc.id} email={email}: {e}"
            )
...
async def quickadd_lines_text(m: types.Message, state: FSMContext):
    ...
    for idx, (acc, email_addr, password, disp) in enumerate(added_accounts):
        try:
            ...
            # ВАЖНО: Все старые UNSEEN письма уже помечены как SEEN в mark_all_unseen_as_read_async.
            # IMAP процесс при первом запуске (last_uid is None) также помечает все UNSEEN как SEEN
            # и устанавливает last_uid на максимальный UID, поэтому старые письма не попадут в очередь.
            # Запускаем IMAP процесс для аккаунта постепенно
            try:
...
async def publish_incoming_to_chat_async(
    ...
    # Пропускаем автоматические письма от AI/роботов
    ...
    if is_blocked:
        try:
            ...
            # Инвалидируем контекст, чтобы он пересоздался без отключенного аккаунта для сендинга
            try:
                invalidate_user_ctx(user_id)
            except Exception as ctx_err:
                log_send_event(f"IMAP: ошибка при инвалидации контекста uid={user_id}: {ctx_err}")

            # ВАЖНО: Отправляем сообщение в чат о отключении аккаунта для массового сендинга
            # Аккаунт продолжает читаться, только отключен для массового сендинга
            try:
                await bot.send_message(
                    chat_id,
                    f"⚠️ Обнаружено письмо {block_reason}, аккаунт {acc_email} отключен для массового сендинга.\n"
                    f"Аккаунт продолжает читаться."
                )
            except Exception as e_msg:
                log_send_event(f"IMAP: error sending message to chat {chat_id}: {e_msg}")

            if was_enabled:
                log_send_event(
                    f"IMAP: обнаружено письмо {block_reason}, аккаунт {acc_email} (acc_id={acc_id}) "
                    f"ПРИНУДИТЕЛЬНО отключен для массового сендинга (был включен вручную)"
                )
            else:
                log_send_event(
                    f"IMAP: обнаружено письмо {block_reason}, аккаунт {acc_email} (acc_id={acc_id}) "
                    f"ПРИНУДИТЕЛЬНО отключен для массового сендинга"
                )
        except Exception as e:
            log_send_event(
                f"IMAP: ошибка при отключении аккаунта для массового сендинга "
                f"uid={user_id} acc_id={getattr(acc, 'id', 'unknown')}: {e}"
            )
        return
    ...
    try:
        ...
        log_send_event(
            f"INCOMING_RT save error uid={user_id} chat_id={chat_id} mid={tg_msg.message_id}: {e_rt}"
        )

    # Второй проход клавиатуры
    try:
    ...
    try:
        ...
        log_send_event(
            f"DB SAVE ERROR incoming uid={user_id} acc={acc.id} tg_mid={tg_msg.message_id}: {e_db}"
        )
...
async def imap_results_drain_loop():
    ...
    imap_runtime кладёт в RESULT_QUEUE кортеж:
    ...
    if q is None:
        log_send_event("IMAP drain: RESULT_QUEUE is None, runtime not initialized")
        return

    MIN_PUBLISH_INTERVAL = 2.0
    last_publish_time = 0.0

    log_send_event("IMAP drain: started")

    # Импортируем QueueEmpty для multiprocessing.Queue
    try:
    ...
    while True:
        try:
            # Используем get_nowait() для неблокирующего чтения из очереди
            # Это проще и эффективнее, чем get(timeout=...) в executor'е
            try:
                item = await loop.run_in_executor(None, q.get_nowait)
            except QueueEmpty:
                # Очередь пуста - ждем немного и проверяем снова
                await asyncio.sleep(0.5)
                continue
        except Exception as e_get:
            log_send_event(f"IMAP drain: error reading from RESULT_QUEUE: {e_get}")
            import traceback
            log_send_event(f"IMAP drain: traceback: {traceback.format_exc()}")
        ...
        try:
            ...
            # 1. Обычные сообщения: (user_id, acc_id, chat_id, email, msgs)
            # 2. Сообщения-ошибки: (user_id, acc_id, chat_id, email, None, "ERROR", error_type, error_msg)
            if len(item) >= 8 and item[5] == "ERROR":
                # Специальное сообщение-ошибка
                user_id, acc_id, chat_id, email_addr, _, _, error_type, error_msg = item[:8]
                msgs = None
                
                # Обрабатываем ошибки:
                # Invalid credentials - ПОЛНОСТЬЮ отключаем аккаунт для чтения и сендинга
                # Другие ошибки (connection, socket, search, timeout) - BACKOFF на 30 минут
                try:
                    from db_async import get_account_async
                    
                    acc = await get_account_async(user_id, acc_id)
                    acc_email = email_addr
                    
                    if error_type == "INVALID_CREDENTIALS":
                        # ВАЖНО: При invalid credentials ПОЛНОСТЬЮ отключаем аккаунт для чтения и сендинга
                        error_desc = "Неверные данные аккаунта"
                        error_icon = "❌"
                        
                        # Отключаем аккаунт для чтения (IMAP)
                        try:
                            imap_runtime.stop_imap_for_account(user_id, acc_id)
                            # Удаляем из кэша IMAP_ACCOUNT_STATUS
                            key = (user_id, acc_id)
                            IMAP_ACCOUNT_STATUS.pop(key, None)
                        except Exception as e_stop:
                            log_send_event(f"IMAP drain: error stopping IMAP for {user_id}/{acc_id}: {e_stop}")
                        
                        # Отключаем аккаунт для сендинга
                        try:
                            await ensure_send_disabled_loaded(user_id)
                            disabled = SEND_DISABLED_ACCOUNTS.setdefault(user_id, set())
                            disabled.add(acc_id)
                            ...
                            log_send_event(f"IMAP drain: error disabling send for {user_id}/{acc_id}: {e_disable}")
                        
                        # Деактивируем аккаунт полностью (set_account_active_async False)
                        try:
                            await set_account_active_async(user_id, acc_id, False)
                        except Exception as e_active:
                            log_send_event(f"IMAP drain: error deactivating account {user_id}/{acc_id}: {e_active}")
                        
                        # Отправляем сообщение в чат о полном отключении аккаунта
                        try:
                            await bot.send_message(
                                chat_id,
                                f"{error_icon} Аккаунт {acc_email}: {error_desc}.\n{error_msg[:200]}\n"
                                f"Аккаунт ПОЛНОСТЬЮ отключен для чтения и сендинга."
                            )
                        except Exception as e_msg:
                            log_send_event(f"IMAP drain: error sending message to chat {chat_id}: {e_msg}")
                        
                        log_send_event(
                            f"IMAP drain: Account {acc_email} (acc_id={acc_id}) INVALID_CREDENTIALS - "
                            f"FULLY DISABLED (reading and sending stopped)"
                        )
                    else:
                        # Для других ошибок - BACKOFF (временная блокировка)
                        if error_type == "CONNECTION_ERROR":
                            error_desc = "Ошибка подключения (таймаут)"
                            error_icon = "⚠️"
                            backoff_duration = "30 минут"
                        else:
                            error_desc = "Ошибка IMAP"
                            error_icon = "⚠️"
                            backoff_duration = "30 минут"
                        
                        # Отправляем сообщение в чат с ошибкой (информационное, аккаунт остается активным)
                        try:
                            await bot.send_message(
                                chat_id,
                                f"{error_icon} Аккаунт {acc_email}: {error_desc}.\n{error_msg[:200]}\n"
                                f"Аккаунт временно в BACKOFF ({backoff_duration}), продолжит работу автоматически."
                            )
                        except Exception as e_msg:
                            log_send_event(f"IMAP drain: error sending message to chat {chat_id}: {e_msg}")
                        
                        log_send_event(
                            f"IMAP drain: Account {acc_email} (acc_id={acc_id}) error {error_type}: {error_msg[:100]} "
                            f"(BACKOFF {backoff_duration}, account remains active)"
                        )
                except Exception as e_error:
                    log_send_event(
                        f"IMAP drain: error processing {error_type} for {user_id}/{acc_id}: {e_error}"
                    )
                continue
            
            # Обычное сообщение с письмами
            user_id, acc_id, chat_id, email_addr, msgs = item[:5]
            
            log_send_event(
                f"IMAP drain: got item from queue uid={user_id} acc_id={acc_id} chat_id={chat_id} "
                f"email={email_addr} msgs_count={len(msgs) if msgs else 0}"
            )
        except Exception as e_unpack:
            log_send_event(f"IMAP drain: bad item in RESULT_QUEUE: {e_unpack}, item={item}")
            continue

        # Обновляем локальный статус аккаунта как активный при получении результатов
        key = (int(user_id), int(acc_id))
        IMAP_ACCOUNT_STATUS[key] = {"active": True, "email": email_addr}

        # Достаём объект аккаунта по user_id/acc_id
        try:
            accounts = await list_accounts_async(user_id)
            acc = next((a for a in accounts if int(getattr(a, "id")) == int(acc_id)), None)
        except Exception as e_acc:
            log_send_event(f"IMAP drain: list_accounts_async failed uid={user_id} acc_id={acc_id}: {e_acc}")
            acc = None

        if acc is None:
            log_send_event(f"IMAP drain: account not found for uid={user_id} acc_id={acc_id}, skip incoming")
            continue

        # Проверяем, что msgs не None и является списком
        if not msgs or not isinstance(msgs, (list, tuple)):
            log_send_event(f"IMAP drain: invalid msgs for uid={user_id} acc_id={acc_id}, msgs={type(msgs)}")
            continue

        # ВАЖНО: Старые UNSEEN письма уже помечены как SEEN при первом запуске IMAP процесса
        # в imap_runtime.py (при last_uid is None). Поэтому все письма в msgs - это новые письма,
        # которые появились ПОСЛЕ добавления аккаунта. Публикуем их все.
        ...
        for uid, raw_bytes in msgs:
            try:
                ...
                # Дедуп до публикации: смотрим в БД, есть ли уже такое входящее
                try:
                    exists = await incoming_message_exists_async(acc.id, uid_str)
                    if exists:
                        # Уже публиковали – не трогаем (не шлём в TG второй раз)
                        continue
                except Exception as e_db_check:
                    log_send_event(
                        f"IMAP drain: DB precheck error user_id={user_id} acc_id={acc_id} "
                        f"uid={uid_str}: {e_db_check}"
                    )
                    # При ошибке БД всё равно продолжаем, чтобы не глохнуть
                ...
                # Ограничиваем скорость публикации: не чаще одного письма в 2 секунды
                current_time = time.time()
                delta = current_time - last_publish_time
                if delta < MIN_PUBLISH_INTERVAL:
                    await asyncio.sleep(MIN_PUBLISH_INTERVAL - delta)

                log_send_event(
                    f"IMAP drain: Publishing message uid={user_id} acc_id={acc_id} chat_id={chat_id} "
                    f"from={from_email} subject={subject[:50]} uid_str={uid_str}"
                )
                ...
                log_send_event(
                    f"IMAP drain: Successfully published message uid={user_id} acc_id={acc_id} "
                    f"from={from_email} uid_str={uid_str}"
                )
...
def _runtime_is_active(uid: int, email: str, acc_id: int = None) -> bool:
    """
    Проверяет, запущен ли IMAP‑runtime для аккаунта пользователя.

    Основной источник истины — imap_runtime.is_imap_account_active(user_id, acc_id).
    Fallback на локальный IMAP_ACCOUNT_STATUS используется только как вторичная эвристика.
    """
    try:
        import imap_runtime
    except Exception:
    ...
    try:
        if acc_id is not None:
            if imap_runtime.is_imap_account_active(uid, int(acc_id)):
                return True
            key = (uid, int(acc_id))
            status = IMAP_ACCOUNT_STATUS.get(key) or {}
            return bool(status.get("active"))

        accounts = ACCOUNTS_CACHE.get(uid)
        if not accounts:
            return False

        for a in accounts:
            acc_email = (getattr(a, "email", "") or "").lower()
            if acc_email == (email or "").lower():
                acc_id = int(getattr(a, "id", 0) or 0)
                if acc_id <= 0:
                    continue
