...
# Используем IMAP IDLE для мгновенных уведомлений вместо polling
# IDLE более эффективен: меньше нагрузки, мгновенные уведомления, меньше трафика
# Fallback на polling при ошибках IDLE или неподдерживаемых серверах

# Интервал polling (используется только при fallback с IDLE)
IMAP_POLL_INTERVAL_MIN = 5.0
IMAP_POLL_INTERVAL_MAX = 5.0

# IDLE настройки
IMAP_IDLE_ENABLED = True              # Использовать IDLE если поддерживается
IMAP_IDLE_TIMEOUT = 1740              # 29 минут (RFC 2177 рекомендует < 30 мин)
IMAP_IDLE_RESTART_INTERVAL = 1740     # Перезапуск IDLE каждые 29 минут (предотвращение таймаутов)
IMAP_IDLE_FALLBACK_TO_POLL = True     # Fallback на polling при ошибках IDLE
...
# - При интервале 5 сек: ~252 опроса/сек общая нагрузка (1261 аккаунт / 5 сек)
# - На процесс: ~12.6 опроса/сек (комфортно для I/O-bound операций при постоянных соединениях)
...
def _check_idle_support(imap: imaplib.IMAP4_SSL) -> bool:
    """
    Проверяет, поддерживает ли сервер IMAP IDLE.
    """
    try:
        ...
        if typ != "OK" or not data:
            return False
        capabilities = data[0].decode() if isinstance(data[0], bytes) else data[0]
        return "IDLE" in capabilities.upper()
    except Exception:
        return False


def _start_idle(imap: imaplib.IMAP4_SSL) -> bool:
    """
    Запускает IMAP IDLE. Возвращает True если успешно.
    """
    try:
        # Используем метод idle() из imaplib если доступен, иначе отправляем вручную
        if hasattr(imap, 'idle'):
            # imaplib.IMAP4.idle() доступен в Python 3.9+
            try:
                imap.idle()
                return True
        ...
        # Fallback: отправляем IDLE вручную
        tag = imap._new_tag()
        cmd = f"{tag} IDLE\r\n".encode('utf-8')
        imap.sock.sendall(cmd)
        
        # Читаем ответ (должен быть "+ idling")
        response = imap.readline()
        if response and b"+ idling" in response.lower():
            return True
        return False
    except Exception as e:
        _log(f"IDLE start error: {e}")
        return False


def _stop_idle(imap: imaplib.IMAP4_SSL) -> bool:
    """
    Останавливает IMAP IDLE (отправляет DONE).
    """
    try:
        # Используем метод done() из imaplib если доступен
        if hasattr(imap, 'done'):
            try:
                typ, data = imap.done()
                return typ == "OK"
            except AttributeError:
        ...
        # Fallback: отправляем DONE вручную
        imap.sock.sendall(b"DONE\r\n")
        
        # Читаем ответ (может быть несколько строк)
        response = imap.readline()
        while response:
            response_str = response.decode('utf-8', errors='ignore').strip()
            if response_str.startswith('*') or response_str.startswith('+') or response_str.startswith('-'):
                if 'OK' in response_str or 'BYE' in response_str:
                    break
            response = imap.readline()
        return True
    except Exception as e:
        _log(f"IDLE stop error: {e}")
        return False


def _wait_idle_notification(imap: imaplib.IMAP4_SSL, timeout: float) -> Optional[str]:
    """
    Ждет уведомление от IDLE (EXISTS, EXPUNGE и т.д.).
    Возвращает тип уведомления или None при таймауте/ошибке.
    
    ВАЖНО: В IDLE режиме сервер отправляет уведомления в формате "* 123 EXISTS"
    без тега команды, поэтому нужно читать напрямую из сокета.
    """
    try:
        if not hasattr(imap, 'sock') or not imap.sock:
            return None
        
        # Используем select для неблокирующего чтения
        ready, _, _ = select.select([imap.sock], [], [], timeout)
        if not ready:
            return None  # Таймаут
        
        # В IDLE режиме уведомления приходят без тега, читаем напрямую из сокета
        # Устанавливаем короткий таймаут для чтения
        old_timeout = imap.sock.gettimeout()
        try:
            imap.sock.settimeout(0.1)
            # Читаем строку напрямую из сокета
            # В IDLE режиме нужно читать через file объект, но он может быть заблокирован
            # Используем прямое чтение из сокета
            line = None
            if hasattr(imap, 'file') and imap.file:
                try:
                    # Пробуем прочитать через file объект (неблокирующе)
                    line = imap.file.readline()
                except Exception:
                    pass
            
            if not line:
                # Fallback: читаем из сокета напрямую
                try:
                    data = imap.sock.recv(4096)
                    if data:
            ...
            if line:
                ...
                _log(f"IDLE notification received: {response_str[:100]}")
                ...
                if "EXISTS" in response_str:
                    ...
                    return "FETCH"
                elif "OK" in response_str or "BYE" in response_str:
                    return "DONE"
        finally:
            if old_timeout is not None:
                try:
                    imap.sock.settimeout(old_timeout)
                except Exception:
                    pass
        ...
        _log(f"IDLE wait notification error: {e}")
        import traceback
        _log(f"IDLE notification traceback: {traceback.format_exc()}")
        return None
...
def _imap_pool_worker(account_queue: Queue,
                      ...
                      account_status: Dict):
    """
    Persistent IMAP IDLE/Polling Runtime Worker:
    - Держит постоянные IMAP соединения для аккаунтов
    - Использует IMAP IDLE для мгновенных уведомлений о новых письмах (если поддерживается)
    - Fallback на polling (SEARCH каждые 5 секунд) при ошибках IDLE или неподдерживаемых серверах
    - Периодически делает NOOP для поддержания соединения (в polling режиме)
    - Перезапускает IDLE каждые 29 минут (RFC 2177)
    ...
    # Словарь активных соединений: key -> (imap, config, last_used, last_uid, use_idle, idle_start_time)
    persistent_connections: Dict[Tuple[int, int], Tuple[imaplib.IMAP4_SSL, ImapAccountConfig, float, Optional[int], bool, float]] = {}
    
    # Словарь конфигураций аккаунтов: key -> config
    account_configs: Dict[Tuple[int, int], ImapAccountConfig] = {}
    
    # Словарь времени последнего опроса: key -> timestamp (для fallback polling)
    last_poll_time: Dict[Tuple[int, int], float] = {}
    
    # Таймауты для очистки соединений
            ...
            for key, config in list(account_configs.items()):
                ...
                if not _can_use_account(config.user_id, config.acc_id, account_status):
                    ...
                    if key in persistent_connections:
                        imap, _, _, _, use_idle, _ = persistent_connections[key]
                        try:
                            if use_idle:
                                _stop_idle(imap)
                            imap.logout()
                ...
                should_check_messages = False  # Флаг: нужно ли проверять новые письма
                
                # Для IDLE: проверяем уведомления, для polling: проверяем интервал
                if key in persistent_connections:
                    stored_imap, _, _, stored_last_uid, use_idle, idle_start_time = persistent_connections[key]
                    
                    if use_idle:
                        # IDLE режим: проверяем уведомления
                        time_since_idle_start = now - idle_start_time
                        if time_since_idle_start >= IMAP_IDLE_RESTART_INTERVAL:
                            # Перезапускаем IDLE каждые 29 минут
                            try:
                                _stop_idle(stored_imap)
                                if _start_idle(stored_imap):
                                    persistent_connections[key] = (stored_imap, config, now, stored_last_uid, True, now)
                                    _log(f"IDLE restarted for {config.user_id}/{config.acc_id}")
                            except Exception as e:
                                _log(f"IDLE restart error for {config.user_id}/{config.acc_id}: {e}")
                                # Fallback на polling
                                persistent_connections[key] = (stored_imap, config, now, stored_last_uid, False, 0.0)
                                use_idle = False
                        
                        if use_idle:
                            # Проверяем уведомления IDLE (неблокирующе)
                            notification = _wait_idle_notification(stored_imap, 0.5)  # Увеличиваем таймаут для надежности
                            if notification == "EXISTS":
                                # Новое письмо - останавливаем IDLE и обработаем
                                _log(f"IDLE EXISTS notification for {config.user_id}/{config.acc_id}, checking for new messages")
                                _stop_idle(stored_imap)
                                imap = stored_imap
                                last_uid = stored_last_uid
                                should_check_messages = True  # ВАЖНО: устанавливаем флаг
                                # Перезапустим IDLE после обработки
                            elif notification in ("EXPUNGE", "FETCH", "RECENT"):
                                # Другое уведомление - останавливаем IDLE и обработаем
                                _log(f"IDLE {notification} notification for {config.user_id}/{config.acc_id}")
                                _stop_idle(stored_imap)
                                imap = stored_imap
                                last_uid = stored_last_uid
                                should_check_messages = True  # ВАЖНО: устанавливаем флаг
                            elif notification == "DONE":
                                # IDLE завершен - перезапустим
                                _log(f"IDLE DONE for {config.user_id}/{config.acc_id}, restarting")
                                try:
                                    if _start_idle(stored_imap):
                                        persistent_connections[key] = (stored_imap, config, now, stored_last_uid, True, now)
                                    else:
                                        _log(f"IDLE restart failed for {config.user_id}/{config.acc_id}, falling back to polling")
                                        persistent_connections[key] = (stored_imap, config, now, stored_last_uid, False, 0.0)
                                        last_poll_time[key] = now
                                except Exception as e:
                                    _log(f"IDLE restart error for {config.user_id}/{config.acc_id}: {e}, falling back to polling")
                                    persistent_connections[key] = (stored_imap, config, now, stored_last_uid, False, 0.0)
                                    last_poll_time[key] = now
                                continue
                            elif notification is None:
                                # Нет уведомлений или таймаут - продолжаем IDLE
                                continue
                            else:
                                # Неизвестное уведомление - обработаем на всякий случай
                                _log(f"IDLE unknown notification {notification} for {config.user_id}/{config.acc_id}")
                                _stop_idle(stored_imap)
                                imap = stored_imap
                                last_uid = stored_last_uid
                                should_check_messages = True  # ВАЖНО: устанавливаем флаг
                    else:
                        # Polling режим: проверяем интервал
                        last_poll = last_poll_time.get(key, 0)
                        time_since_last_poll = now - last_poll
                        if time_since_last_poll < IMAP_POLL_INTERVAL_MIN:
                            # Делаем NOOP для поддержания соединения
                            try:
                            ...
                            # Время для polling - используем существующее соединение
                            imap = stored_imap
                            last_uid = stored_last_uid
                            should_check_messages = True  # ВАЖНО: устанавливаем флаг
                
                # Получаем или создаем соединение (только если еще не установлено)
                if imap is None:
                    need_reconnect = True
                
                # Если соединение еще не установлено (например, при переподключении)
                if imap is None and key in persistent_connections:
                    stored_imap, stored_config, last_used, stored_last_uid, stored_use_idle, stored_idle_start = persistent_connections[key]
                    ...
                    if idle_time > IMAP_CONNECTION_IDLE_TIMEOUT or idle_time > IMAP_CONNECTION_MAX_AGE:
                        ...
                        try:
                            if stored_use_idle:
                                # IDLE режим - соединение активно (уже обработано выше)
                                imap = stored_imap
                                last_uid = stored_last_uid
                            else:
                                # Polling режим - проверяем через NOOP
                                stored_imap.sock.settimeout(IMAP_NOOP_TIMEOUT)
                                typ, _ = stored_imap.noop()
                                if typ == "OK":
                                    imap = stored_imap
                                    last_uid = stored_last_uid
                                    should_check_messages = True  # ВАЖНО: устанавливаем флаг для polling
                                else:
                                    need_reconnect = True
                        except Exception:
                ...
                if need_reconnect or imap is None:
                    ...
                    try:
                        ...
                        use_idle = False
                        if IMAP_IDLE_ENABLED:
                            try:
                                # Выбираем папку INBOX для IDLE
                                typ, _ = imap.select("INBOX")
                                if typ == "OK":
                                    use_idle = _check_idle_support(imap)
                                    if use_idle:
                                        _log(f"IDLE supported for {config.user_id}/{config.acc_id} ({config.email})")
                                    else:
                                        _log(f"IDLE not supported for {config.user_id}/{config.acc_id} ({config.email}), using polling")
                            except Exception as e:
                                _log(f"IDLE check error for {config.user_id}/{config.acc_id}: {e}, using polling")
                                use_idle = False
                        ...
                        # Запускаем IDLE если поддерживается
                        idle_start_time = 0.0
                        if use_idle:
                            try:
                                if _start_idle(imap):
                                    idle_start_time = now
                                    _log(f"IDLE started for {config.user_id}/{config.acc_id} ({config.email})")
                                else:
                                    _log(f"IDLE start failed for {config.user_id}/{config.acc_id}, using polling")
                                    use_idle = False
                            except Exception as e:
                                _log(f"IDLE start error for {config.user_id}/{config.acc_id}: {e}, using polling")
                                use_idle = False
                        
                        # Сохраняем соединение
                        persistent_connections[key] = (imap, config, now, last_uid, use_idle, idle_start_time)
                        
                        # ВАЖНО: При reconnect всегда проверяем новые письма (на случай если пропустили)
                        should_check_messages = True
                    except Exception as e:
                        _log(f"Connection error for {config.user_id}/{config.acc_id}: {e}")
                        error_msg = str(e)
                        _register_error(config.user_id, config.acc_id, error_msg, account_status)
                        
                        # Определяем тип ошибки
                        error_msg_lower = error_msg.lower()
                        is_invalid_credentials = (
                            "invalid credentials" in error_msg_lower or
                            "invalid credential" in error_msg_lower or
                            "authentication failed" in error_msg_lower or
                            "auth failed" in error_msg_lower or
                            "login failed" in error_msg_lower
                        )
                        ...
                        # Отправляем ошибку в очередь для обработки главным процессом
                        try:
                            result_queue.put(
                                (config.user_id, config.acc_id, config.chat_id, config.email, None, "ERROR", error_type, error_msg)
                            )
                        except Exception as e_put:
                            _log(f"Pool worker {worker_id}: failed to put error in result queue for {config.user_id}/{config.acc_id}: {e_put}")
                        
                        _release_connection()
                        continue
                
                # Обрабатываем аккаунт: проверяем новые письма
                # ВАЖНО: Проверяем только если установлен флаг или при reconnect/polling
                if imap is None:
                    continue
                
                # ВАЖНО: Проверяем новые письма только если нужно (should_check_messages или reconnect)
                if not should_check_messages and not need_reconnect:
                    # IDLE режим без уведомлений - пропускаем
                    continue
                ...
                try:
                    ...
                    # ВАЖНО: Если last_uid еще не установлен, берем из status
                    if last_uid is None:
                        last_uid = st.get("last_uid")
                    ...
                    # Получаем состояние IDLE
                    use_idle = False
                    idle_start_time = 0.0
                    if key in persistent_connections:
                        _, _, _, _, use_idle, idle_start_time = persistent_connections[key]
                    
                    # ВАЖНО: Проверяем новые письма (SEARCH)
                    _log(f"Pool worker {worker_id}: checking for new messages uid={config.user_id} acc_id={config.acc_id} last_uid={last_uid}")
                    uids = _search_new_uids(imap, last_uid)
                    
                    # Инициализируем переменную для last_uid
                    current_last_uid = last_uid
                    ...
                    if uids:
                        ...
                        if msgs:
                            ...
                            _log(f"Pool worker {worker_id}: no messages fetched for {config.user_id}/{config.acc_id} (uids={len(uids)})")
                    else:
                        _log(f"Pool worker {worker_id}: no new messages for {config.user_id}/{config.acc_id} (last_uid={last_uid})")
                    
                    # Обновляем соединение и перезапускаем IDLE если нужно
                    # ВАЖНО: Обновляем last_uid в persistent_connections даже если писем не было
                    
                    if use_idle:
                        try:
                            # Перезапускаем IDLE после обработки (даже если писем не было)
                            if _start_idle(imap):
                                persistent_connections[key] = (imap, config, now, current_last_uid, True, now)
                                _log(f"IDLE restarted after processing for {config.user_id}/{config.acc_id} (last_uid={current_last_uid})")
                            else:
                                # Fallback на polling
                                _log(f"IDLE restart failed for {config.user_id}/{config.acc_id}, falling back to polling")
                                persistent_connections[key] = (imap, config, now, current_last_uid, False, 0.0)
                                last_poll_time[key] = now
                        except Exception as e:
                            _log(f"IDLE restart error for {config.user_id}/{config.acc_id}: {e}, falling back to polling")
                            import traceback
                            _log(f"IDLE restart error traceback: {traceback.format_exc()}")
                            persistent_connections[key] = (imap, config, now, current_last_uid, False, 0.0)
                            last_poll_time[key] = now
                    else:
                        # Polling режим - обновляем время последнего опроса
                        last_poll_time[key] = now
                        persistent_connections[key] = (imap, config, now, current_last_uid, False, 0.0)
                    
                    # ВАЖНО: Для IDLE режима также делаем периодическую проверку (fallback на случай если IDLE не сработал)
                    # Проверяем каждые 30 секунд даже в IDLE режиме для надежности
                    if use_idle:
                        last_poll = last_poll_time.get(key, 0)
                        if now - last_poll > 30.0:  # Fallback проверка каждые 30 секунд
                            _log(f"IDLE fallback check for {config.user_id}/{config.acc_id} (30s interval) last_uid={last_uid}")
                            # Останавливаем IDLE, проверяем письма, затем перезапускаем
                            try:
                                _stop_idle(imap)
                                # ВАЖНО: Используем актуальный last_uid из status
                                fallback_last_uid = st.get("last_uid") or last_uid
                                # Проверяем новые письма
                                fallback_uids = _search_new_uids(imap, fallback_last_uid)
                                if fallback_uids:
                                    _log(f"IDLE fallback: found {len(fallback_uids)} new messages for {config.user_id}/{config.acc_id}")
                                    fallback_msgs = _fetch_messages(imap, fallback_uids)
                                    if fallback_msgs:
                                        new_last_uid = max(fallback_last_uid or 0, max(fallback_uids))
                                        st["last_uid"] = new_last_uid
                                        st["last_ok_ts"] = now
                                        # Помечаем как прочитанные
                                        try:
                                            uid_sequence = ",".join(str(uid) for uid in fallback_uids)
                                            imap.uid("STORE", uid_sequence, "+FLAGS", r"(\Seen)")
                                        except Exception:
                                            pass
                                        # Отправляем в очередь
                                        try:
                                            result_queue.put(
                                                (config.user_id, config.acc_id, config.chat_id, config.email, fallback_msgs)
                                            )
                                            _log(f"IDLE fallback: sent {len(fallback_msgs)} messages to result queue for {config.user_id}/{config.acc_id}")
                                        except Exception as e_fallback_put:
                                            _log(f"IDLE fallback: failed to put messages in queue for {config.user_id}/{config.acc_id}: {e_fallback_put}")
                                        last_uid = new_last_uid
                                else:
                                    _log(f"IDLE fallback: no new messages for {config.user_id}/{config.acc_id} (last_uid={fallback_last_uid})")
                                # Перезапускаем IDLE
                                if _start_idle(imap):
                                    persistent_connections[key] = (imap, config, now, last_uid, True, now)
                                    _log(f"IDLE restarted after fallback check for {config.user_id}/{config.acc_id}")
                                else:
                                    _log(f"IDLE restart failed after fallback check for {config.user_id}/{config.acc_id}, falling back to polling")
                                    persistent_connections[key] = (imap, config, now, last_uid, False, 0.0)
                                    last_poll_time[key] = now
                            except Exception as e:
                                _log(f"IDLE fallback check error for {config.user_id}/{config.acc_id}: {e}")
                                import traceback
                                _log(f"IDLE fallback check traceback: {traceback.format_exc()}")
                            last_poll_time[key] = now
                    ...
                    if "EOF" in error_str or "socket error" in error_str:
                        ...
                        if key in persistent_connections:
                            try:
                                _, _, _, _, use_idle, _ = persistent_connections[key]
                                if use_idle:
                                    _stop_idle(imap)
                            except:
                                pass
    ...
    for key, (imap, _, _, _, use_idle, _) in list(persistent_connections.items()):
        try:
            if use_idle:
                _stop_idle(imap)
            imap.logout()
