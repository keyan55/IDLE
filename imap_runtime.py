    ...
    while not stop_event.is_set():
        try:
            ...
            for key, config in list(account_configs.items()):
                ...
                if key in persistent_connections:
                    stored_imap, _, _, _, use_idle, idle_start_time = persistent_connections[key]
                    if use_idle:
                        ...
                        if time_since_idle_start >= IMAP_IDLE_RESTART_INTERVAL:
                            ...
                            try:
                                ...
                                if _start_idle(stored_imap):
                                    persistent_connections[key] = (stored_imap, config, now, persistent_connections[key][3], True, now)
                                    _log(f"IDLE restarted for {config.user_id}/{config.acc_id}")
                            except Exception as e:
                                _log(f"IDLE restart error for {config.user_id}/{config.acc_id}: {e}")
                                # Fallback на polling
                                persistent_connections[key] = (stored_imap, config, now, persistent_connections[key][3], False, 0.0)
                                use_idle = False
                        ...
                        if use_idle:
                            ...
                            if notification == "EXISTS":
                                ...
                                _stop_idle(stored_imap)
                                imap = stored_imap
                                last_uid = persistent_connections[key][3]
                ...
                if key in persistent_connections and imap is None:
                ...
                try:
                    ...
                    if use_idle:
                        try:
                            # Перезапускаем IDLE после обработки
                            if _start_idle(imap):
                                persistent_connections[key] = (imap, config, now, new_last_uid if uids else last_uid, True, now)
                                _log(f"IDLE restarted after processing for {config.user_id}/{config.acc_id}")
                            else:
                                # Fallback на polling
                                _log(f"IDLE restart failed for {config.user_id}/{config.acc_id}, falling back to polling")
                                persistent_connections[key] = (imap, config, now, new_last_uid if uids else last_uid, False, 0.0)
                                last_poll_time[key] = now
                        except Exception as e:
                            _log(f"IDLE restart error for {config.user_id}/{config.acc_id}: {e}, falling back to polling")
                            persistent_connections[key] = (imap, config, now, new_last_uid if uids else last_uid, False, 0.0)
                    ...
                    if use_idle:
                        ...
                        if now - last_poll > 30.0:  # Fallback проверка каждые 30 секунд
                            _log(f"IDLE fallback check for {config.user_id}/{config.acc_id} (30s interval)")
                            # Останавливаем IDLE, проверяем письма, затем перезапускаем
                            try:
                                _stop_idle(imap)
                                # Проверяем новые письма
                                fallback_uids = _search_new_uids(imap, last_uid)
                                if fallback_uids:
                                    ...
                                    if fallback_msgs:
                                        new_last_uid = max(last_uid or 0, max(fallback_uids))
                                        st["last_uid"] = new_last_uid
                                        # Помечаем как прочитанные
                                        try:
                                ...
                                if _start_idle(imap):
                                    ...
                                    _log(f"IDLE restart failed after fallback check for {config.user_id}/{config.acc_id}")
                            except Exception as e:
